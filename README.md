...
| **Concrete Localized PAC-Bayes Instantiation** | Medium | Instantiate `stabilityPacBayesBound_provability` with an actual `StabilityCertificate` (e.g., the linear layer certificate) and a concrete bounded loss, proving the full pipeline: layer certificate → localized Gibbs posterior → generalization bound. |
