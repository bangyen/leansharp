...
| :--- | :--- | :--- |
| **Paper-Faithful Mask Direction** | High | The paper *keeps* the components with the **largest** absolute Z-scores ($m_j = 1$ iff $\lvert\Omega(\nabla L(w))_j\rvert > q_{Q_p}$), amplifying statistically significant directions. `zScoreMask` keeps components **within** the threshold, i.e. the exact complement. The repo's inlier semantics is load-bearing for the robustness half of the library (`FilterBias`, `BreakdownPoint`, the median comparisons), so this is not a comparator flip: it needs a second, outlier-keeping filter defined alongside the existing one, with the convergence results restated over it. |
| **Percentile Threshold** | Medium | The paper's threshold $q_{Q_p}$ is a *percentile* of the batch of $\lvert$Z-scores$\rvert$, so a fixed fraction of coordinates survives regardless of the gradient's distribution. `zScoreMask` uses a fixed multiplier $z\cdot\sigma$, under which the retained count varies. Requires an order-statistic / rank formalization over `Finset` that mathlib does not provide directly. Independent of the mask-direction row. |
| **Layer-Wise Filter Statistics** | Medium | The paper computes $\mu, \sigma$ per layer $\ell$. `zScoreMask` takes a single mean and std over the whole flat index, so `zsharpStep` filters globally. Per-layer filtering already exists in `backpropChain` (`Core/Models.lean`), but the convergence path does not use it; connecting the `Chain` architecture to the update is the actual work. |
...
computed globally rather than per layer. The inlier semantics is deliberate — the robustness results depend
on it — but it means the convergence theorems here are results about an outlier-*suppressing* variant, not
about the paper's outlier-amplifying algorithm.
