...
structure ZSharpModel (ι : Type*) [Fintype ι] where
  ...
  alignment : ∀ w : W ι,
    ...
    alignment_condition L.toFun w w_star ε z L.μ (L.smoothness : ℝ)
...
theorem zsharp_convergence (M : ZSharpModel ι) (η : Schedule)
    ...
    zsharp_convergence_holds M.L.toFun M.w_star η M.ρ M.z (M.L.smoothness : ℝ) M.L.μ := by
  let L_obj := M.L
  let w_star := M.w_star
  let ρ := M.ρ
  let z := M.z
  ...
  have h_c_valid : ∀ t, 0 < c t ∧ c t < 1 := by
    ...
    · have hη_nonneg : 0 ≤ η t := (le_of_lt (hη t))
      ...
      have h_eta_mul_mu_le : η t * L_obj.μ ≤ (L_obj.μ / ((L_obj.smoothness : ℝ) ^ 2)) * L_obj.μ :=
  ...
  rw [zsharp_step]
  let ε := sam_perturbation L_obj.toFun w ρ
  let g_f := filtered_gradient (gradient L_obj.toFun (w + ε)) z
  ...
  have h_main : ‖w - w_star‖ ^ 2 - 2 * η t * inner ℝ g_f (w - w_star) + η t ^ 2 * ‖g_f‖ ^ 2 ≤
    ...
    calc ‖w - w_star‖ ^ 2 - 2 * η t * inner ℝ g_f (w - w_star) + η t ^ 2 * ‖g_f‖ ^ 2
      ...
      _ = (1 - 2 * η t * L_obj.μ + η t ^ 2 * (L_obj.smoothness : ℝ) ^ 2) * ‖w - w_star‖ ^ 2 := by
        rw [sq]; ring
