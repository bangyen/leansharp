...
theorem localized_filtered_update_norm_bound
  ...
  calc
    ...
    _ = η * ‖g‖ := by
      simp only [sub_sub_cancel_left, norm_neg, norm_smul]
      rw [Real.norm_eq_abs, abs_of_nonneg hη]
    _ ≤ η * R := mul_le_mul_of_nonneg_left hR hη
...
theorem localized_filtered_update_norm_bound_of_reference
  ...
  have h_norm_g : ‖g‖ ≤ R_ref + Δ := by
    calc
      ...
      _ = R_ref + Δ := by ring
  exact localized_filtered_update_norm_bound w g η z (R_ref + Δ) hη h_norm_g
...
theorem uniform_filtered_process_stability
  ...
  | succ t ih =>
      have h_inc : ‖w (t + 1) - w t‖ ≤ η t * R := by
        rw [h_step t]
        exact localized_filtered_update_norm_bound (w t) (g t) (η t) z R (hη t) (hR t)
