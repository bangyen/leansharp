...
theorem median_breakdown_lower_bound {ι : Type*} [Fintype ι] {α : Type*}
    (s_out : Finset α) (v m : W ι) (R C : ℝ)
    (h_norm_le : ‖m‖ ≤ C) (h_norm_v : ‖v‖ = R) :
    ∑ _ ∈ s_out, ‖v - m‖ ≥ (s_out.card : ℝ) * (R - C) := by
  have h1' : ∑ i ∈ s_out, ‖v - m‖ ≥ ∑ _ ∈ s_out, (R - C) := by
    apply Finset.sum_le_sum; intro i _
    have H := norm_sub_norm_le v m
    rw [h_norm_v] at H
    have hR_C : R - C ≤ R - ‖m‖ := sub_le_sub_left h_norm_le R
    exact hR_C.trans H
  rw [Finset.sum_const (R - C), nsmul_eq_mul] at h1'
  exact h1'

theorem median_breakdown_upper_bound {ι : Type*} [Fintype ι] {α : Type*}
    (s_fixed : Finset α) (g : α → W ι) (v : W ι) (R : ℝ)
    (h_norm_v : ‖v‖ = R) :
    ∑ i ∈ s_fixed, ‖g i - v‖ ≤ (∑ i ∈ s_fixed, ‖g i‖) + (s_fixed.card : ℝ) * R := by
  trans ∑ i ∈ s_fixed, (‖g i‖ + ‖v‖)
  · exact Finset.sum_le_sum (fun i _ => norm_sub_le (g i) v)
  · simp only [Finset.sum_add_distrib, Finset.sum_const, nsmul_eq_mul, h_norm_v]; rfl
...
theorem median_breakdown [Nonempty ι] (s : Finset α) (g : α → W ι) (s_fixed : Finset α)
  ...
  · set g' := fun i => if i ∈ s_out then R • unitVector ι else g i
    ...
    have h_decomp : s = s_fixed ∪ s_out := by
      rw [Finset.union_sdiff_of_subset h_sub]
    have h_disj : Disjoint s_fixed s_out := Finset.disjoint_sdiff
    -- Goal: ∑ i ∈ s, ‖g' i - m‖ ≥ n_out * (R - C)
    have h_m_sum_ge : ∑ i ∈ s, ‖g' i - m‖ ≥ n_out * (R - C) := by
      rw [h_decomp, Finset.sum_union h_disj]
      have h_lower := median_breakdown_lower_bound s_out v m R C h_norm_le h_norm_v
      have h_m_sum_fixed : ∑ i ∈ s_fixed, ‖g' i - m‖ = ∑ i ∈ s_fixed, ‖g i - m‖ := by
        apply Finset.sum_congr rfl; intro i hi; rw [hg'_fixed i hi]
      have h_m_sum_out : ∑ i ∈ s_out, ‖g' i - m‖ = ∑ i ∈ s_out, ‖v - m‖ := by
        apply Finset.sum_congr rfl; intro i hi; rw [hg'_out i hi]
      rw [h_m_sum_fixed, h_m_sum_out]
      have h2 : 0 ≤ ∑ i ∈ s_fixed, ‖g i - m‖ := Finset.sum_nonneg (fun _ _ => norm_nonneg _)
      exact (zero_add (n_out * (R - C))).symm.le.trans (add_le_add h2 h_lower)
    -- Goal: ∑ i ∈ s, ‖g' i - v‖ = ∑ i ∈ s_fixed, ‖g i - v‖
    have h_sum_at_v : ∑ i ∈ s, ‖g' i - v‖ = ∑ i ∈ s_fixed, ‖g i - v‖ := by
      rw [h_decomp, Finset.sum_union h_disj]
      have h_v_sum_fixed : ∑ i ∈ s_fixed, ‖g' i - v‖ = ∑ i ∈ s_fixed, ‖g i - v‖ := by
        apply Finset.sum_congr rfl; intro i hi; rw [hg'_fixed i hi]
      have h_v_sum_out : ∑ i ∈ s_out, ‖g' i - v‖ = 0 := by
        rw [Finset.sum_eq_zero]; intro i hi; rw [hg'_out i hi, sub_self, norm_zero]
      rw [h_v_sum_fixed, h_v_sum_out, add_zero]
    have h_v_ub := median_breakdown_upper_bound s_fixed g v R h_norm_v
    have h_m_le_v : ∑ i ∈ s, ‖g' i - m‖ ≤ ∑ i ∈ s, ‖g' i - v‖ := hm_min v
    rw [h_sum_at_v] at h_m_le_v
    have key : R * (n_out - n_fixed) ≤ n_out * C + B := by
      have := h_m_sum_ge.trans (h_m_le_v.trans h_v_ub)
      nlinarith
