  ...
  | .append p_relu_linear w2 =>
    ...
    | .append p_linear w_relu =>
      ...
      | .single _ w1 =>
        ...
        have h1 : ∃ K : NNReal,
            LipschitzWith K (fun x => (linearLayer ι_in ι_mid).forward w1 x) := by
          classical
          ...
          have h_affine (x : W ι_in) :
              (linearLayer ι_in ι_mid).forward w1 x = L x + b := by
            ext i
            simp only [
              linearLayer,
              linearForward,
        ...
        have h2 : LipschitzWith 1 (fun x => (reluLayer ι_mid).forward w_relu x) := by
        ...
        have h3 : ∃ K : NNReal,
            LipschitzWith K (fun x => (linearLayer ι_mid ι_out).forward w2 x) := by
          classical
          ...
          have h_affine (x : W ι_mid) :
              (linearLayer ι_mid ι_out).forward w2 x = L x + b := by
            ext i
            simp only [
              linearLayer,
              linearForward,
