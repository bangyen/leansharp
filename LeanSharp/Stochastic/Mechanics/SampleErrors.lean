...
* `norm_filtered_gradient_le`: Proves the $L_2$ norm contraction of the filter.
...
theorem zsharp_variance_bound (L : W ι → ℝ) (g_adv : Ω → W ι) (w : W ι) (z σsq : ℝ)
    (h_unbiased : IsStochasticGradient L g_adv w)
    (h_base_var : HasBoundedVariance L g_adv w σsq)
    (h_int_fg : Integrable (fun ω => ‖filteredGradient (g_adv ω) z‖ ^ 2))
    (h_int_g : Integrable (fun ω => ‖g_adv ω‖ ^ 2)) :
    𝔼[fun ω => ‖filteredGradient (g_adv ω) z‖ ^ 2] ≤ σsq + ‖gradient L w‖ ^ 2 := by
  calc 𝔼[fun ω => ‖filteredGradient (g_adv ω) z‖ ^ 2]
      ≤ 𝔼[fun ω => ‖g_adv ω‖ ^ 2] :=
        integral_mono h_int_fg h_int_g (fun ω => norm_sq_filtered_gradient_le (g_adv ω) z)
