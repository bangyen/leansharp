...
private theorem stochastic_taylor_expectation_rhs {ι : Type*} [Fintype ι]
    {Ω : Type*} [MeasureSpace Ω] [IsProbabilityMeasure (volume : Measure Ω)]
    (f : W ι → ℝ) (g : Ω → W ι) (w : W ι) (η L_smooth : ℝ)
...
private theorem stochastic_taylor_eta_bound (η L_smooth term_grad term_var f_w : ℝ)
    (h_η : 0 < η ∧ η ≤ 1 / L_smooth) (h_grad_nonneg : 0 ≤ term_grad) :
    f_w - η * term_grad + (η ^ 2 * L_smooth / 2) * (term_var + term_grad) ≤
    f_w - (η / 2) * term_grad + (η ^ 2 * L_smooth / 2) * term_var := by
...
theorem stochastic_taylor_descent (L_smooth : ℝ) (f : W ι → ℝ) (g : Ω → W ι)
  ...
  rw [stochastic_taylor_expectation_rhs f g w η L_smooth h_stoch h_int] at h_int_le
  ...
  calc 𝔼[fun ω => f (w - η • g ω)]
    ...
    _ ≤ f w - (η / 2) * term_grad + (η ^ 2 * L_smooth / 2) * term_var :=
        stochastic_taylor_eta_bound η L_smooth term_grad term_var (f w) h_η (sq_nonneg _)
