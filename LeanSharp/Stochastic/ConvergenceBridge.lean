...
omit [Fintype ι] [IsProbabilityMeasure (volume : Measure Ω)] in
/-- Converts a uniform objective-space `L¹` bound into the transformed-process
`L¹` bound used by the `-f` convergence bridge. -/
theorem zsharp_neg_uniform_l1_of_objective_uniform_l1
    (f : W ι → ℝ) (w : ℕ → Ω → W ι) (R : NNReal)
...
omit [Fintype ι] [IsProbabilityMeasure (volume : Measure Ω)] in
/-- Converts an objective-space one-step supermartingale inequality into the
transformed-process one-step inequality used by the `-f` bridge. This exists so
callers can state one-step descent in natural objective coordinates. -/
theorem zsharp_neg_step_mono_of_objective_supermartingale_step
    (f : W ι → ℝ) (w : ℕ → Ω → W ι)
    (ℱ : Filtration ℕ ‹MeasureSpace Ω›.toMeasurableSpace)
    (h_step_obj :
      ∀ t, ℙ[fun ω => f (w (t + 1) ω) | ℱ t] ≤ᵐ[ℙ] (fun ω => f (w t ω))) :
    ∀ t, (fun ω => -f (w t ω)) ≤ᵐ[ℙ] ℙ[fun ω => -f (w (t + 1) ω) | ℱ t] := by
  intro t
  have hneg_obj :
      (fun ω => -f (w t ω)) ≤ᵐ[ℙ] (fun ω => -ℙ[fun ω => f (w (t + 1) ω) | ℱ t] ω) := by
    filter_upwards [h_step_obj t] with ω hω
    linarith
  have hcond_neg :
      (fun ω => -ℙ[fun ω => f (w (t + 1) ω) | ℱ t] ω) =ᵐ[ℙ]
        ℙ[fun ω => -f (w (t + 1) ω) | ℱ t] := by
