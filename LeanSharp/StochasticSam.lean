...
lemma stochastic_sgd_contraction {b : ℕ} (η L_smooth μ σ : ℝ) (w_star : W d)
    ...
    ∀ w : W d, ∀ B ∈ batches,
        ∃ c : ℝ, 0 < c ∧ c < 1 ∧
        ‖(w - η • minibatch_gradient sample_loss S B w) - w_star‖^2 ≤
            c * ‖w - w_star‖^2 + η^2 * σ^2 := by
  intro w B hB
  obtain ⟨c, ⟨hc_pos, hc_lt_1⟩, _h_det⟩ := gd_contraction (fun v => empirical_risk sample_loss S v)
...
theorem stochastic_zsharp_convergence {b : ℕ} (η ρ z L_smooth μ σ : ℝ) (w_star : W d)
    ...
    ∀ w : W d, ∀ B ∈ batches,
        ∃ c : ℝ, 0 < c ∧ c < 1 ∧
        ‖stochastic_zsharp_update sample_loss S B w η ρ z - w_star‖^2 ≤
            c * ‖w - w_star‖^2 + η^2 * σ^2 := by
  sorry
